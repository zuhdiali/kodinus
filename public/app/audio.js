document.addEventListener('DOMContentLoaded', () => {
  ...
  audioForm.addEventListener("submit", async (event) => {
    ...
    const loading = audioForm.querySelector('.loading');
    const errorMessage = audioForm.querySelector('.error-message');
    const sentMessage = audioForm.querySelector('.sent-message');
    ...
    formData.append("file_audio", recordedAudioBlob, "pesan-suara.wav");
    ...
    try {
      fetch("http://localhost:8000/api/berbicara", {   // <-- GANTI DENGAN URL SERVER ANDA
        method: "POST",
        body: formData,
        headers: {
      ...
      .catch(error => {
        ...
        errorMessage.innerHTML = 'Terjadi kesalahan saat mengirim jawaban.';
